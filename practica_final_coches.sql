...
create table vehiculos.marca(
	...
	constraint grupo_fk foreign key (id_grupo) references vehiculos.grupo(id)
);
...
create table vehiculos.modelo(
	...
	constraint marca_fk foreign key (id_marca) references vehiculos.marca(id)
);
...
create table vehiculos.vehiculo(
	...
	constraint modelo_fk foreign key (id_modelo) references vehiculos.modelo(id),
	constraint color_fk foreign key (id_color) references vehiculos.color(id)
);
...
create table vehiculos.poliza(
	...
	constraint aseguradora_fk foreign key (id_aseguradora) references vehiculos.aseguradora(id),
...
create table vehiculos.revision(
	...
	constraint moneda_fk foreign key (id_moneda) references vehiculos.moneda(id),
...
insert into vehiculos.grupo (nombre) select grupo from vehiculos.coches group by grupo order by grupo;

insert into vehiculos.marca  (nombre,id_grupo) select coches.marca,grupo.id from vehiculos.coches 
inner join vehiculos.grupo on grupo.nombre= coches.grupo
group by coches.marca,grupo.id order by grupo.id;


insert into vehiculos.modelo  (nombre,id_marca) select coches.modelo,marca.id from vehiculos.coches 
inner join vehiculos.marca on marca.nombre= coches.marca
group by coches.modelo,marca.id order by marca.id;


insert into vehiculos.color (nombre) select color from vehiculos.coches group by color order by color;

insert into vehiculos.vehiculo  (matricula,id_modelo,fecha_compra,km,id_color) select coches.matricula,modelo.id,coches.fecha_compra,coches.kms_totales,color.id  from vehiculos.coches 
inner join vehiculos.modelo on modelo.nombre = coches.modelo
inner join vehiculos.color on color.nombre = coches.color
group by coches.matricula,modelo.id,coches.fecha_compra,coches.kms_totales,color.id order by coches.fecha_compra;
...
insert into vehiculos.moneda (nombre,cod_iso) select moneda,
				case when moneda = 'Dólar' then 'USD' 
					 ...
					 else 'N/D' end as cod_iso from vehiculos.coches group by moneda order by moneda;
...
insert into vehiculos.poliza (num_poliza,id_aseguradora,id_vehiculo,fecha_poliza) select coches.n_poliza,aseguradora.id,vehiculo.id,coches.fecha_alta_seguro 
...
group by coches.n_poliza,aseguradora.id,vehiculo.id,coches.fecha_alta_seguro order by vehiculo.id,coches.fecha_alta_seguro ;

insert into vehiculos.revision (id_vehiculo,fecha_revision,km_revision,importe_revision,id_moneda) select vehiculo.id,coches.fecha_revision,coches.kms_revision,coches.importe_revision,moneda.id

from vehiculos.coches 
inner join  vehiculos.vehiculo  on vehiculo.matricula = coches.matricula 
inner join vehiculos.moneda on moneda.nombre = coches.moneda

group by vehiculo.id,coches.fecha_revision,coches.kms_revision,coches.importe_revision,moneda.id
...
select 	grupo.nombre as grupo,
		...
		inner join vehiculos.color on vehiculo.id_color = color.id
		inner join vehiculos.modelo on vehiculo.id_modelo = modelo.id
		inner join vehiculos.marca on modelo.id_marca = marca.id
		inner join vehiculos.grupo on marca.id_grupo = grupo.id
		...
		inner join vehiculos.aseguradora on poliza.id_aseguradora = aseguradora.id;
